...
class BlackAndWhite(Problem):
    __NEIGHBOURS = ((-1, 0), (1, 0), (0, -1), (0, 1))

    def __init__(self, initial, board_size):
        self.board_size = board_size
        super().__init__(initial)

    def give_opposite_color(self, color):
    ...
    def color_block(self, state, element, i, j):

        new_state = list(map(lambda row: list(row), state))
        new_state[i][j] = self.give_opposite_color(element)

        for move_to_neighbour in BlackAndWhite.__NEIGHBOURS:
            neighbour_i, neighbour_j = i + move_to_neighbour[0], j + move_to_neighbour[1]
            if self.valid_neighbour(neighbour_i, neighbour_j):
                new_state[neighbour_i][neighbour_j] = self.give_opposite_color(new_state[neighbour_i][neighbour_j])

        return tuple(map(lambda row: tuple(row), new_state))

    def successor(self, state):
        successors_states = dict()

        for i, row in enumerate(state):
            for j, element in enumerate(state[i]):
                new_state = self.color_block(state, element, i, j)
                if new_state:
                    successors_states[f'x: {i}, y: {j}'] = new_state
    ...
    def goal_test(self, state):
        return sum(map(lambda row: sum(row), state)) == (self.board_size + 1) ** 2


if __name__ == "__main__":
    n = int(input())
    full_array = list(map(int, input().split(",")))
    start = 0
    matrix = []
    for i in range(n):
        matrix.append(tuple(full_array[start:start + n]))
        start += n
    problem = BlackAndWhite(tuple(matrix), n - 1)
