...
class GhostOnSkates(Problem):
    __MOVES = {"Gore 1": (0, 1), "Gore 2": (0, 2), "Gore 3": (0, 3), "Desno 1": (1, 0),
               "Desno 2": (2, 0), "Desno 3": (3, 0), }
    ...
    def check_valid(state, walls, n):
        ...
        if state in walls:
            return False
        if not is_on_board((n,n), state):
            return False

        return True

    def move_ghost(self, state, where):

        move = GhostOnSkates.__MOVES[where]
        new_state = list(state)
        new_state = [new_state[0] + move[0], new_state[1] + move[1]]

        if not GhostOnSkates.check_valid(tuple(new_state),self.walls,self.n - 1):
            return Node
    ...
    def successor(self, state):
        ...
        for where in GhostOnSkates.__MOVES:
            new_state = self.move_ghost(state,where)
            if new_state is not Node:
                successors[where] = new_state

        return successors

    def h(self, node):
        state = node.state
        value = manhattan_distance(state,self.goal) / 3
        return value
