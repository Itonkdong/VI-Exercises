...
def secret(num_greens, greens, search):
    if num_greens == 2:
        print(
    ...
    elif num_greens == 3 and greens[0] == (0, 4):
        print(
...
class Snake(Problem):
    ...
    def h(self, node):
        state = node.state
        body, direction, green_apples_booleans = state
        from_position = body[0]

        value = 0
        for index, green_apple_boolean in enumerate(green_apples_booleans):
            if green_apple_boolean == 0:
                continue

            new_from = self.green_apples[index]
            value += manhattan_distance(from_position, new_from)
            from_position = new_from
        return value

        # return sum(green_apples_booleans)
...
if __name__ == '__main__':
    ...
    snake_problem = Snake(green_apples)
    search = astar_search(snake_problem)
    print(search)
    if search:
        print(search.solution())
