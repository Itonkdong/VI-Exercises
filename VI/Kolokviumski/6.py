...
class Squares(Problem):
    __MOVES = {"gore": (0, 1), "dolu": (0, -1), "levo": (-1, 0), "desno": (1, 0)}
    ...
    def move_square(self, state, square, where, index_of_square):

        move = Squares.__MOVES[where]
        new_state = list(state)
        new_square = list(square)
        new_square = [new_square[0] + move[0], new_square[1] + move[1]]
        new_state[index_of_square] = tuple(new_square)
        if not Squares.check_valid(tuple(new_state)):
            return None
    ...
    def successor(self, state):
        ...
        for index, square in enumerate(state):
            for where in Squares.__MOVES:
                new_state = self.move_square(state, square, where, index)
                if new_state:
                    successors_states[f"Pomesti kvadratche {index + 1} {where}"] = new_state
...
if __name__ == '__main__':
    ...
    search = breadth_first_graph_search(squares)
    # print(search)
    if search:
